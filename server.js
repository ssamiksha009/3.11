...
async function generateBatchFile(projectName, protocol) {
    try {
        ...
        for (const test of testData) {
            ...
            // Around line 3142 - FIXED: Declare subfolderPath and rootPath BEFORE the CDTire block

// Normalize P and L values FIRST (before any folder path logic)
...
// ✅ CRITICAL: Declare these BEFORE the if statement so they're in scope
const subfolderPath = path.join(projectPath, folderName); // e.g., demo3_CDTire/P2_L1
const rootPath = projectPath; // e.g., demo3_CDTire

// Determine folder path
let folderPath;

if (protocol === 'CDTire') {
    // STEP 2: Check where .inp file exists (subfolder FIRST, then root)
    ...
    // Check subfolder FIRST (ALL .inp files for jobs are here)
    if (fs.existsSync(subfolderInpPath)) {
        folderPath = subfolderPath;
        console.log(`Found ${inpFileName} in subfolder: ${folderName}`);
    } 
    // Then check root (for supporting .inp files like 0_axi_mesh.inp, road.inp)
    else if (fs.existsSync(rootInpPath)) {
        folderPath = rootPath;
        console.log(`Found ${inpFileName} in root: ${projectPath}`);
    } 
    // Fallback: Use subfolder path (most .inp files are there)
    else {
        folderPath = subfolderPath;
        console.warn(`WARNING: ${inpFileName} not found in subfolder or root, defaulting to ${folderName}`);
    }
} else {
    // OTHER PROTOCOLS: Use P_L subfolders (no change)
...
if (protocol === 'CDTire') {
    ...
    // ✅ FIXED: Check if job .inp exists, otherwise use old_job .inp
    const jobInpFileName = jobName.endsWith('.inp') ? jobName : `${jobName}.inp`;
    const oldJobInpFileName = oldJobName ? (oldJobName.endsWith('.inp') ? oldJobName : `${oldJobName}.inp`) : null;
    
    // Check which .inp file to use (NOW subfolderPath is in scope)
    const subfolderJobInpPath = path.join(subfolderPath, jobInpFileName);
    const rootJobInpPath = path.join(rootPath, jobInpFileName);
    const subfolderOldJobInpPath = oldJobInpFileName ? path.join(subfolderPath, oldJobInpFileName) : null;
    const rootOldJobInpPath = oldJobInpFileName ? path.join(rootPath, oldJobInpFileName) : null;
    
    let inputFileToUse = null;
    let jobToRun = jobBaseName;
    
    // Priority 1: Check for job .inp in subfolder (original test files)
if (fs.existsSync(subfolderJobInpPath)) {
    inputFileToUse = jobInpFileName;
    folderPath = subfolderPath;
    console.log(`Found ${jobInpFileName} in subfolder: ${folderName}`);
}
// Priority 2: Check for job .inp in ROOT (generated files)
else if (fs.existsSync(rootJobInpPath)) {
    inputFileToUse = `..\\${jobInpFileName}`;  // ✅ Reference from parent directory
    folderPath = subfolderPath;  // Still run from P2_L1
    console.log(`Found ${jobInpFileName} in root (generated file)`);
}
// Priority 3: Check for old_job .inp in subfolder
else if (subfolderOldJobInpPath && fs.existsSync(subfolderOldJobInpPath)) {
    inputFileToUse = oldJobInpFileName;
    jobToRun = oldJobBaseName;
    folderPath = subfolderPath;
    console.log(`Job .inp not found, using old_job: ${oldJobInpFileName} in subfolder`);
}
    // Priority 4: Check for old_job .inp in root
    else if (rootOldJobInpPath && fs.existsSync(rootOldJobInpPath)) {
        inputFileToUse = oldJobInpFileName;
        jobToRun = oldJobBaseName;
        folderPath = rootPath;
        console.log(`Job .inp not found, using old_job: ${oldJobInpFileName} in root`);
    }
    // Fallback: Use job name anyway (will fail, but shows clear error)
    else {
        inputFileToUse = jobInpFileName;
        folderPath = subfolderPath;
        console.warn(`WARNING: Neither ${jobInpFileName} nor ${oldJobInpFileName} found, defaulting to ${folderName}`);
    }
    
    // ✅ BUILD ABAQUS COMMAND WITH CORRECT INPUT FILE
    batchContent += `REM Run ${runNumber}\n`;
    batchContent += `cd /d "${folderPath}"\n`;
    
    // Build command: job → oldjob → input → user → cpus → interactive
    let abaqusCmd = `call abaqus job=${jobToRun}`;
    
    // Add oldjob if specified AND different from job being run
    if (oldJobBaseName && oldJobBaseName !== '-' && oldJobBaseName !== jobToRun) {
        abaqusCmd += ` oldjob=${oldJobBaseName}`;
    }
    
    // ✅ CRITICAL: Add input parameter
    abaqusCmd += ` input=${inputFileToUse}`;
    
    // Add Fortran file if present
    if (fortranFile && fortranFile.trim() !== '' && fortranFile !== '-') {
        const fortranBaseName = fortranFile.replace('.f', '');
        
        // Check if we're in subfolder - if so, reference root
        if (folderPath.includes('P2_L1') || folderPath.includes('P1_L1')) {
    ...
    // Around line 3230 - FIXED: Reference parent directory for Python scripts

// STEP 2: Run Python script if present
if (pythonScript && pythonScript.trim() !== '' && pythonScript !== '-') {
    const pythonBaseName = pythonScript.replace('.py', '');
    
    // ✅ CRITICAL FIX: Python scripts are in project root, not subfolder
    // If we're in P2_L1, reference parent directory for Python script
    let pythonScriptPath = pythonBaseName;
    
    if (folderPath.includes('P2_L1') || folderPath.includes('P1_L1') || folderPath.includes('_L')) {
        // We're in a subfolder, so reference parent directory
        pythonScriptPath = `..\\${pythonBaseName}`;
    }
    
    batchContent += `call abaqus python ${pythonScriptPath}.py ${jobBaseName}.odb\n`;
}
    ...
    // OTHER PROTOCOLS: Standard workflow
    const jobBaseName = jobName.replace('.inp', '');
