...
function initLogsSidebar() {
  console.log('📋 Initializing logs sidebar...');
  
  // Setup event listeners
  document.getElementById('refreshLogsSidebarBtn')?.addEventListener('click', refreshLogsSidebar);
  document.getElementById('clearLogsBtn')?.addEventListener('click', clearLogsSidebar);
  document.getElementById('toggleLogsBtn')?.addEventListener('click', toggleLogsSidebar);
  document.getElementById('downloadLogsBtn')?.addEventListener('click', downloadLogsSidebar);
  document.getElementById('copyLogsSidebarBtn')?.addEventListener('click', copyLogsSidebar);
  ...
  if (logsContent) {
    logsContent.addEventListener('scroll', () => {
      ...
      if (logsAutoScroll) {
        logsContent.classList.add('auto-scrolling');
      } else {
        logsContent.classList.remove('auto-scrolling');
      }
    });
  }
...
function startLiveLogUpdates() {
  if (logsUpdateInterval) {
    console.log('⚠️ Log updates already running');
    return;
  }
  
  console.log('▶️ Starting live log updates...');
  
  const projectName = sessionStorage.getItem('currentProject');
  const protocol = getProtocolFromCurrentTable();
  
  if (!projectName || !protocol) {
    console.warn('Cannot start log updates: missing project info');
    return;
  }
  
  // Initial load
  loadLogsSidebar(projectName, protocol);
  
  // Update every 2 seconds
  logsUpdateInterval = setInterval(() => {
    loadLogsSidebar(projectName, protocol, true);
  }, 2000);
  
  // Update status
  updateLogsStatus('running', 'Live');
  
  console.log('✅ Live log updates started');
}
...
function stopLiveLogUpdates() {
  if (logsUpdateInterval) {
    ...
    console.log('⏹️ Live log updates stopped');
    
    // Update status
    updateLogsStatus('idle', 'Idle');
  }
}
 ...
 */
async function loadLogsSidebar(projectName, protocol, isAutoUpdate = false) {
  const logsContent = document.getElementById('logs-content');
  ...
  try {
    ...
    if (data.success && data.content) {
      const newSize = data.content.length;
      ...
      if (newSize !== lastLogSize) {
        ...
        logsContent.innerHTML = formatLogsForSidebar(data.content);
        ...
        if (!isAutoUpdate) {
          console.log(`📄 Logs loaded: ${data.filename} (${newSize} bytes)`);
        }
      }
...
/**
 * Format logs for sidebar display
 */
function formatLogsForSidebar(content) {
  if (!content) return '';
  ...
  return content
    ...
    .map(line => {
      ...
      const timestampMatch = trimmed.match(/^\[(\d{4}-\d{2}-\d{2}T[\d:\.]+Z)\]/);
      const timestamp = timestampMatch ? timestampMatch[1] : '';
      const message = timestamp ? trimmed.substring(timestampMatch[0].length).trim() : trimmed;
      
      // Determine log level
      let className = 'log-entry';
      if (message.includes('[ERROR]') || message.includes('❌') || message.includes('Error')) {
        className += ' error';
      } else if (message.includes('[SUCCESS]') || message.includes('✅') || message.includes('SUCCESS')) {
        className += ' success';
      } else if (message.includes('[WARN]') || message.includes('⚠️') || message.includes('WARNING')) {
        className += ' warning';
      } else if (message.includes('[INFO]') || message.includes('ℹ️')) {
        className += ' info';
      }
      
      const timestampHtml = timestamp ? `<span class="log-timestamp">${formatTimestamp(timestamp)}</span>` : '';
      
      return `<div class="${className}">${timestampHtml}${escapeHtml(message)}</div>`;
    })
    .filter(Boolean)
    .join('');
}
 ...
 */
function updateLogsStatus(state, text) {
 ...
 */
function updateLogsTimestamp() {
  const timestamp = document.getElementById('logs-timestamp');
  if (timestamp) {
    const now = new Date();
...
/**
 * Refresh logs sidebar
 */
async function refreshLogsSidebar() {
  const btn = document.getElementById('refreshLogsSidebarBtn');
  if (btn) {
  ...
  if (projectName && protocol) {
    await loadLogsSidebar(projectName, protocol);
  }
...
/**
 * Clear logs sidebar (visual only)
 */
function clearLogsSidebar() {
  const logsContent = document.getElementById('logs-content');
  if (logsContent) {
    logsContent.innerHTML = `
      <div class="logs-empty">
        <i class="fas fa-info-circle"></i>
        <p>Logs cleared</p>
        <small>New logs will appear when tests run</small>
      </div>
    `;
...
/**
 * Toggle logs sidebar collapse/expand
 */
function toggleLogsSidebar() {
  const sidebar = document.getElementById('logs-sidebar');
  const icon = document.querySelector('#toggleLogsBtn i');
 ...
 */
async function downloadLogsSidebar() {
  const projectName = sessionStorage.getItem('currentProject');
  const protocol = getProtocolFromCurrentTable();
  ...
  try {
    ...
    if (data.success && data.content) {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = data.filename || 'project_logs.log';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
...
/**
 * Copy logs sidebar to clipboard
 */
async function copyLogsSidebar() {
  const logsContent = document.getElementById('logs-content');
  if (!logsContent) return;
  ...
  try {
    ...
    const btn = document.getElementById('copyLogsSidebarBtn');
    if (btn) {
...
// Initialize logs sidebar on page load
window.addEventListener('DOMContentLoaded', () => {
  initLogsSidebar();
  
  // Load initial logs
  const projectName = sessionStorage.getItem('currentProject');
  const protocol = sessionStorage.getItem('currentProjectProtocol') || getProtocolFromCurrentTable();
...
// Start live updates when batch file runs
document.getElementById('runBatchFileBtn')?.addEventListener('click', () => {
  // Start live log updates after batch starts
  setTimeout(() => {
    startLiveLogUpdates();
  }, 1000);
});

// Stop live updates when user navigates away
